    ...
    ReentrancyGuardUpgradeable
{
    ...
    function swapAndStakeAssetsToNode(
        ...
        require(nodeId < nodes.length, "Invalid node");
        // Handle ETH wrapping if needed
        if (tokenIn == ETH_ADDRESS) {
            require(msg.value == amountIn, "ETH mismatch");
            IWETH(WETH).deposit{value: amountIn}();
            tokenIn = WETH; // Work with WETH
        }

        // Get swap instructions from library
        FinalAutoRoutingLib.SwapInstructions memory inst = FinalAutoRoutingLib
            .getAutoSwapInstructions(
                tokenIn,
                tokenOut,
                amountIn,
                minAmountOut,
        ...
        // Execute swap based on instructions
        if (inst.needsApproval) {
            IERC20(tokenIn).approve(inst.approvalTarget, inst.approvalAmount);
        }
        ...
        (bool success, ) = inst.target.call{value: inst.value}(inst.callData);
        ...
        require(amountOut >= inst.guaranteedAmountOut, "Slippage exceeded");

        // Clean up approval
        if (inst.needsApproval) {
            IERC20(tokenIn).approve(inst.approvalTarget, 0);
        }

        // Handle ETH unwrapping if needed
        if (tokenOut == WETH && nodes[nodeId].acceptsETH) {
            IWETH(WETH).withdraw(amountOut);
            tokenOut = ETH_ADDRESS;
        }

        // Stake the output
        _stakeAssetsToNode(nodeId, tokenOut, amountOut);
        ...
        emit SwapAndStake(
            ...
            amountOut,
            nodeId,
            inst.strategy == FinalAutoRoutingLib.SwapStrategy.QuoterFirst
        );
    }

    function executeMultiStepSwapAndStake(
        address tokenIn,
        address tokenOut,
        uint256 amountIn,
        uint256 minAmountOut,
        ...
        // Get multi-step instructions
        FinalAutoRoutingLib.MultiStepInstructions
            memory inst = FinalAutoRoutingLib.getMultiStepSwapInstructions(
                tokenIn,
                tokenOut,
                amountIn,
                minAmountOut,
        ...
        // Execute each step
        for (uint256 i = 0; i < inst.steps.length; i++) {
            FinalAutoRoutingLib.SwapInstructions memory step = inst.steps[i];

            if (step.needsApproval && step.approvalAmount > 0) {
                // For multi-step, amounts might need recalculation
                uint256 actualAmount = step.approvalAmount;
                if (actualAmount == 0 && i > 0) {
                    // Use output from previous step
                    actualAmount = IERC20(getCurrentToken(i, tokenIn, inst))
                        .balanceOf(address(this));
                }
                IERC20(getCurrentToken(i, tokenIn, inst)).approve(
                    step.approvalTarget,
                    actualAmount
                );
            }
        ...
        // Verify final output
        uint256 finalAmount = tokenOut == ETH_ADDRESS
            ? address(this).balance
            : IERC20(tokenOut).balanceOf(address(this));
        require(finalAmount >= minAmountOut, "Insufficient output");

        // Stake
        _stakeAssetsToNode(nodeId, tokenOut, finalAmount);
    }
    */
