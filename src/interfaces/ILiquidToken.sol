...
interface ILiquidToken is IERC20 {
    ...
    // === PRICE UPDATE EVENTS AND ERRORS ===
    // New events for price updates
    event PricesUpdatedDuringDeposit(address indexed user, bool updated);
    event PriceUpdateFailed(string reason);

    // New errors for price updates
    error PriceUpdateError(string reason);
