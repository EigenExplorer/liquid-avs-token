...
import "../../src/interfaces/ILSTSwapRouter.sol";
...
contract MockLiquidTokenManager is ReentrancyGuard {
    ...
    function initialize(Init memory init) external {
        require(address(LSTswaprouter) == address(0), "Already initialized");
        LSTswaprouter = ILSTSwapRouter(init.LSTswaprouter);
        weth = init.weth;
    }

    // Admin function to update LSR
    function updateLSTSwapRouter(address newLSTSwapRouter) external {
        require(newLSTSwapRouter != address(0), "Zero address");
    ...
    /// @dev Flow: MockLTM >> DEX >> MockLTM (using LSR for routing data)
    function _swapAndStakeAssetsToNode(
        ...
        for (uint256 i = 0; i < assetsLength; i++) {
            ...
            if (tokenIn == tokenOut) {
                ...
                // Execute swap using LSR
                uint256 actualAmountOut = _executeLSRSwapPlan(tokenIn, tokenOut, amountIn);
    ...
    /// @dev Executes a swap plan from LSR following MockLTM >> DEX >> MockLTM flow
    function _executeLSRSwapPlan(
        address tokenIn,
        address tokenOut,
        uint256 amountIn
    ) internal returns (uint256 actualAmountOut) {
        console.log("\n[MockLTM] Executing LSR swap plan:");
        console.log("Token In:", tokenIn);
        console.log("Token Out:", tokenOut);
        console.log("Amount In:", amountIn);
    ...
    function _executeStepByStepSwap(address tokenIn, address tokenOut, uint256 amountIn) internal returns (uint256) {
        ...
        while (currentTokenIn != tokenOut) {
            ...
            (uint256 quotedOutput, ILSTSwapRouter.MultiStepExecutionPlan memory plan) = LSTswaprouter
            ...
            // Use the first step
            ILSTSwapRouter.SwapStep memory firstStep = plan.steps[0];
    ...
    function getLSTSwapRouter() external view returns (address) {
        return address(LSTswaprouter);
    }
