    ...
    if (web3 && account) {
      try {
        ...
        if (avgGasPerTx > 0) {
          ...
          const ethPerTx = new BN(avgGasPerTx).multipliedBy(new BN(price)).div(new BN(10).pow(18));
          const estimatedRemainingTxs = new BN(balance).div(new BN(avgGasPerTx).multipliedBy(new BN(price)));
...
export async function updateIndividualPrice(
  ...
  try {
    ...
    // Record gas usage for this individual transaction
  ...
  if (validUpdates.length > 0) {
    ...
    while (retries > 0) {
      try {
        ...
        logger.info(`Estimated cost: ${Web3.utils.fromWei(new BN(gas).multipliedBy(gasPrice).toString(), 'ether')} ETH`);
        ...
        // Record gas usage for this batch transaction
        gasTracker.recordTransaction(
          receipt.transactionHash, 
          parseInt(receipt.gasUsed), 
          gasPrice, 
